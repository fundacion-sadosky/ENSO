...
onMounted(() => {
  ...
  if (!route.path.startsWith('/404')) {
    window.location.href = `/app5/404${route.fullPath}`
  }
})
    ...
    <div class="error-container">
      <div class="error-wrapper">
        <div class="error-inner has-text-centered">
          ...
          <SVGErrorPlaceholder />
          <h3>No pudimos encontrar esa página.</h3>
          <p>
            Parece que no pudimos encontrar esa página. Inténtelo de nuevo o comuníquese con un administrador si el
            problema persiste.
          </p>
          <div class="button-wrap">
            <VButton color="primary" elevated :to="{ name: 'index' }"> Volver </VButton>
          </div>
        </div>
      </div>
    </div>
