...
const loadMachines = async () => {
  try {
    const qry: any = { flts: {} }
    let ret = await modelService.getMachines(qry)
...
<template>
  <div class="list-view-toolbar">
    ...
    <!-- <VButtons align="right">
      <VButton :to="{ name: 'machine-edit', params: { objId: 'new' } }" color="primary" icon="fas fa-plus" raised elevated>
        Add App
      </VButton>
    </VButtons> -->
  </div>
        ...
        <div v-for="obj in filteredData" :key="obj.id" class="column is-4">
          <div class="tile-grid-item">
            <div class="tile-grid-item-inner">
              <RouterLink :to="{ name: 'machine-edit', params: { objId: obj.id } }">
                <VIconBox v-if="obj.realtime_status" size="medium" color="primary" rounded>
                  <i class="iconify" data-icon="icon-park-outline:washing-machine"></i>
                </VIconBox>
                <VIconBox v-if="!obj.realtime_status" size="medium" color="yellow" rounded>
                  <i class="iconify" data-icon="icon-park-outline:washing-machine"></i>
                </VIconBox>
              </RouterLink>
              <div class="meta">
                <RouterLink :to="{ name: 'machine-edit', params: { objId: obj.id } }">
                  <span class="dark-inverted">{{ obj.nbr }} - {{ obj.code }}</span>
                  <span>{{ obj.name }}</span>
                </RouterLink>
              </div>
