...
const onAddFileModel = async (error: any, fileInfo: any) => {
  if (error) {
    console.error(error)
    return
  }
  ...
  if (_file) {
    ...
    try {
      ...
      const platformFile: any = await platformService.addAppFile(objId, 'GAMS_SOURCE', formData)
      if (platformFile && theObj.value) {
        theObj.value.file_resource_gams_source = platformFile
...
const onAddFileInputSample = async (error: any, fileInfo: any) => {
  if (error) {
    console.error(error)
    return
  }
  ...
  if (_file) {
    ...
    try {
      const objId = route.params.objId as string
      const platformFile: any = await platformService.addAppFile(objId, 'INPUT_SAMPLE', formData)
      if (platformFile && theObj.value) {
        theObj.value.file_resource_input_sample = platformFile
        notyf.success('File uploaded!')
      }
    } catch (error: any) {
      console.log(error)
...
const onAddFileOutputSample = async (error: any, fileInfo: any) => {
  if (error) {
    console.error(error)
    return
  }
  ...
  if (_file) {
    ...
    try {
      const objId = route.params.objId as string
      const platformFile: any = await platformService.addAppFile(objId, 'OUTPUT_SAMPLE', formData)
      if (platformFile && theObj.value) {
        theObj.value.file_resource_output_sample = platformFile
        notyf.success('File uploaded!')
      }
    } catch (error: any) {
      console.log(error)
        ...
        <div class="form-section">
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField>
                <VControl>
                  <VSwitchBlock v-model="theObj.use_emulator" color="primary" label="Use Emulator" />
                </VControl>
              </VField>
            </div>
            ...
            <div class="column is-12">
              <h4 class="title is-6 is-narrow has-text-primary">Related Files</h4>
            </div>
              ...
              <VField>
                <VControl v-tooltip="'Drop file here or Browse'">
                  <VFilePond
                    ...
                    style-button-process-item-position="center bottom"
                    icon-remove="<i class='iconify' data-icon='feather:check-circle' aria-hidden='true'></i>"
                    @addfile="onAddFileModel"
                  />
                  <p>Upload gms file if app run a GAMS problem solving</p>
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField>
                <VControl v-tooltip="'Drop file here or Browse'">
                  <VFilePond
                    ...
                    style-button-process-item-position="center bottom"
                    icon-remove="<i class='iconify' data-icon='feather:check-circle' aria-hidden='true'></i>"
                    @addfile="onAddFileInputSample"
                  />
                  <p>Upload INPUT sample file</p>
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField>
                <VControl v-tooltip="'Drop file here or Browse'">
                  <VFilePond
                    ...
                    style-button-process-item-position="center bottom"
                    icon-remove="<i class='iconify' data-icon='feather:check-circle' aria-hidden='true'></i>"
                    @addfile="onAddFileOutputSample"
                  />
                  <p>Upload OUTPUT Sample file</p>
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
              <h4 class="title is-6 is-narrow has-text-primary">Current files</h4>
            </div>
            <AppFiles :app-def="theObj" />
            <div class="column is-12">
              <h4 class="title is-6 is-narrow has-text-primary">Admin</h4>
            </div>
