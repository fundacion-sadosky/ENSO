...
##############################################################################
def function_model(lista_id,operarios,ots,maquinas,fuera_servicios,
...
fecha_inicio_plan = datetime.strptime('15/09/2023 00:00', '%d/%m/%Y %H:%M')

model_input = {
        'tiempo_setup': 0.5,
        'cantidad_operarios': 10,
        'ots': [
            {'id': 1,
             'ot_nro': "24597",
             'maquina_nro': "24",
             ...
             'cantidad': 5760,
             'horas': 34.9,
             'fecha_vencimiento': datetime.strptime('10/09/2023', '%d/%m/%Y'),
             'cadencia': 165,
        ...
        'fuera_servicios': [
            {'id': 1,
             'maquina': "8",
             'hora_inicio': datetime.strptime('04/09/2023 10:00', '%d/%m/%Y %H:%M'),
             'hora_fin': datetime.strptime('04/09/2023 16:00', '%d/%m/%Y %H:%M'),
            },
            {'id': 1,
             'maquina': "10",
             'hora_inicio': datetime.strptime('02/09/2023 10:00', '%d/%m/%Y %H:%M'),
             'hora_fin': datetime.strptime('02/09/2023 16:00', '%d/%m/%Y %H:%M'),
            },
        ]
    }
...
fecha_entrega = {ot_nro: (fecha - fecha_inicio_plan).total_seconds(
    ) / 3600 +24 for ot_nro, fecha in fecha_vencimiento.items()}
...
for item in model_input['fuera_servicios']:
    ...
    diferencia_horas = (hora_inicio - fecha_inicio_plan
                        ).total_seconds() / 3600

    tiinactiva[maquina] = float(diferencia_horas)

    diferencia_horas = (hora_fin - fecha_inicio_plan
                        ).total_seconds() / 3600
    ...
    if fs['maquina'] == ot_prioridad_1['maquina_nro']:
        if tiinactiva[fs['maquina']] < ot_prioridad_1['horas']:
              ...
              model_output = function_model(lista_id,operarios,ots,maquinas,fuera_servicios,
                              operarios_requeridos,asignacion,prioridad, proc,
                              fecha_entrega,tiinactiva,tfinactiva,changeover)
              print('===> Model output')
              print(model_output)
...
ots = model_input.get('ots', [])
# Crear una lista de todos los IDs
ids = [ot.get('id') for ot in ots]
...
ots = model_input.get('ots', [])
fuera_servicios = model_input.get('fuera_servicios', [])
# Crear un conjunto de todas las máquinas en 'ots'
maquinas_en_ots = set(ot.get('maquina_nro') for ot in ots)
# Verificar si las máquinas en 'fuera_servicios' están en 'maquinas_en_ots'
maquinas_fuera_de_servicio = [fs.get('maquina') for fs in fuera_servicios if fs.get('maquina') not in maquinas_en_ots]
...
dicc_validaciones['Validación_5']
...
dicc_validaciones['Validación_7']
