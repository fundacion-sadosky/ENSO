...
fecha_inicio_plan = datetime.strptime('04/09/2023 00:00', '%d/%m/%Y %H:%M')
model_input = {
        'tiempo_setup': 0.5,
...
for fs in model_input['fuera_servicios']:
    if fs['maquina'] == ot_prioridad_1['maquina_nro']:
        if tiinactiva[fs['maquina']] < ot_prioridad_1['horas']:
                ...
                print(mensaje_infactible)
        else:
            model_output = function_model(lista_id,operarios,ots,maquinas,
                           fuera_servicios,operarios_requeridos,asignacion,
                           prioridad, proc,fecha_entrega,tiinactiva,tfinactiva,
                           changeover)
            print('===> Model output')
            print(model_output)