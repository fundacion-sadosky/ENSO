...
from app0.app5.model import JobRunLog
from app0.app5.util import IDX_JOB_LOG
...
async def _get_jobs(es, query: Query, offset: int = 0, page_size: int = 20) -> SearchResults:
    ...
    assert query
    count: int
    sort_field = query.sort.field if query.sort else 'line_date'
    sort_order = query.sort.order if query.sort else 1

    if query.flts:
        search_query = query.find_qry()
        cursor = es[IDX_JOB_LOG].find(search_query)
        count = await es[IDX_JOB_LOG].count_documents(search_query)
    else:
        cursor = es[IDX_JOB_LOG].find()
        count = await es[IDX_JOB_LOG].estimated_document_count()

    cursor = cursor.sort(sort_field, sort_order)
    results = [Payload.from_obj(doc, JobRunLog) for doc in await cursor.skip(offset).to_list(length=page_size)]
