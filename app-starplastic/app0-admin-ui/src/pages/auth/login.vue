...
const doLogin = async () => {
  if (!isLoading.value) {
    ...
    if (res.loggedIn) {
      notif.success(`Bienvenido ${username.value}`)
      ...
      } else if (redirect && redirect.startsWith(APP_APP2_PREFIX)) {
        // window.open(redirect, '_self')
        window.location.href = redirect
      } else if (redirect && redirect.startsWith(APP_APP3_PREFIX)) {
        // window.open(redirect, '_self')
        window.location.href = redirect
      } else if (redirect && redirect.startsWith(APP_APP4_PREFIX)) {
        // window.open(redirect, '_self')
        window.location.href = redirect
      } else if (redirect && redirect.startsWith(APP_APP5_PREFIX)) {
        // window.open(redirect, '_self')
        window.location.href = redirect
      } else if (redirect && redirect.startsWith(APP_APP6_PREFIX)) {
        // window.open(redirect, '_self')
        window.location.href = redirect
      } else {
        router.push('home')
      ...
      if (status === 401) {
        notif.error('Usuario o contraseña incorrecta. Por favor, revise los datos e intente nuevamente')
      } else {
        notif.error('Error inesperado')
      }
    }
          ...
          <div class="hero-heading">
            ...
            <div class="auth-logo has-text-centered">
              <RouterLink :to="{ name: 'index' }" class="header-item">
                ...
                <h2>Platforma de aplicaciones</h2>
              </RouterLink>
            </div>
          </div>
          <div class="hero-body">
                ...
                <div class="column is-12">
                  <div class="auth-content">
                    <div class="form-text" :class="[step !== 'login' && 'is-hidden']">
                      ...
                      <VMessage color="info"> Ingrese usuario y contraseña para acceder al sistema </VMessage>
                    </div>
                    <div class="form-text" :class="[step !== 'forgot-password' && 'is-hidden']">
                      <h2>Recuperación de contraseña</h2>
                      <p>Restablecer la contraseña de su cuenta.</p>
                    </div>
                    <div class="form-text" :class="[step !== 'recovery' && 'is-hidden']">
                      <h2>Email enviado!</h2>
                      <p>Revise su casilla de correo electrónico, por favor.</p>
                      <VMessage color="primary"
                        ><div>
                          <span>
                            Le enviamos un correo electrónico detallando los pasos para completar el procedimiento de
                  ...
                  <div class="auth-form-wrapper">
                    ...
                    <form :class="[step !== 'login' && 'is-hidden']" @submit.prevent="doLogin">
                      <div class="login-form">
                        ...
                        <div class="forgot-link has-text-right">
                          <a @click="step = 'forgot-password'">¿Olvidó su contraseña?</a>
                        </div>
                        <!-- Switch
                    ...
                    <form :class="[step !== 'forgot-password' && 'is-hidden']" class="login-wrapper" @submit.prevent>
                      <VMessage color="primary">
                        <div>
                          <span
                            >Se enviará un correo electrónico detallando los pasos para completar el procedimiento de
                      ...
                      <VField>
                        <VControl icon="feather:mail" :has-error="vErrors.emailOk !== ''">
                          <input
                            ...
                            autocomplete="username"
                          />
                          <small>Su correo electrónico es requerido.</small>
                        </VControl>
                      </VField>
                    ...
                    <form :class="[step !== 'recovery' && 'is-hidden']" class="login-wrapper" @submit.prevent>
                      <div>
                        <VButtons>
                          <VButton color="primary" size="big" lower fullwidth @click="step = 'login'"
                            >Continuar</VButton
                          >
                        </VButtons>
                      </div>
...
<style lang="scss">
/*! _auth.scss | Vuero | Css ninja 2020-2021 */
